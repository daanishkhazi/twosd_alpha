                ...
                <ul tabIndex={0} className="dropdown-content menu p-2 border-4 border-primary-400 shadow-2xl bg-base-100 rounded-box w-52">
                  <li>
                    <div className="lg:mt-0 text-sm text-gray-700 hover:text-primary-500">
                      <progress className="progress justify-right h-4" value={(session.user.tokensUsed/session.user.tokenQuota)*100} max="100">{`${(session.user.tokensUsed/session.user.tokenQuota)*100}%`}</progress>
                      {/* TODO - need to think about how to present this and increment this with queries instead of just on session change */}
                      <a className="text-xs">{`${session.user.tokensUsed/session.user.tokenQuota}%`}</a>
                    </div>
                  </li>
                  <li>
                    <div className="items-center lg:mt-0 text-sm text-gray-700 hover:text-primary-500">
                      <Settings/>
