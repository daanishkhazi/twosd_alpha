...
const InterfaceAnimation = (props: {sample_query_outputs: Array<Array<string>>}) => {
    ...
    const animationPrompts = [{description: "What's the difference between..."}, {description: "Explain the intuition behind..."}, {description: "Explain why this is wrong:"}, {description: "Finish the thought:"}, {description: "Ask an open ended question"}]
    const [selectedPrompt, setSelectedPrompt] = useState<Prompt | null>(animationPrompts[4])
    ...
    useEffect(() => {
        ...
        const effectWrapper = async () => {
            ...
            if (currentQuery.length === endQuery.length) {
                setCurrExampleIndex((currExampleIndex + 1) % sample_query_outputs.length)
        ...
        currentQuery.length === 0 ? setTimeout(() => effectWrapper(), 600) : effectWrapper()
        ...
        const effectWrapper = async () => {
            if (!firstRender) {
                const timeout2 = setTimeout(() => {
                    ...
                    if (currExampleIndex === 0) { setCurrRenderedIndex(-1); }
                    setSendAnimationTrigger("");
                    setSelectedPrompt({description: sample_query_outputs[(currExampleIndex)][2]});
    ...
    return (
        ...
        <div className={`flex w-[42rem] h-[28rem] border-8 border-secondary-400 ${currRenderedIndex >= 1 ? "items-end" : "items-center"} bg-white px-14 relative overflow-hidden`}>
        <div className={`flex mask-up ${currRenderedIndex >= 1 ? "items-end" : "items-center"} w-[42rem] h-[28rem] items-end`}>
        <div className={`flex-row w-full relative pointer-events-none z-0`}>
            {currRenderedIndex >= 0 && <QueryOutputAnimation sample_query_outputs={[sample_query_outputs[0]]} dynamic={false} textSize={"text-sm"}/>}
            {currRenderedIndex >= 1 && <QueryOutputAnimation sample_query_outputs={[sample_query_outputs[1]]} dynamic={false} textSize={"text-sm"}/>}
            {currRenderedIndex >= 2 && <QueryOutputAnimation sample_query_outputs={[sample_query_outputs[2]]} dynamic={false} textSize={"text-sm"}/>}
